...
127.0.0.1 server.emop.emopdata.com
127.0.0.1 frontend.emop.emopdata.com
127.0.0.1 foundation.emop.emopdata.com
127.0.0.1 plm.emop.emopdata.com
127.0.0.1 datahub.emop.emopdata.com
127.0.0.1 cad-integration.emop.emopdata.com

# 基础服务域名映射
127.0.0.1 emop.emopdata.com
127.0.0.1 graph.emop.emopdata.com
127.0.0.1 fulltext.emop.emopdata.com
127.0.0.1 storage.emop.emopdata.com
127.0.0.1 minioproxy.emop.emopdata.com
127.0.0.1 keycloak.emop.emopdata.com
127.0.0.1 documentserver.emop.emopdata.com
127.0.0.1 kafka.emop.emopdata.com
127.0.0.1 camunda-zeebe.emop.emopdata.com
127.0.0.1 camunda-operate.emop.emopdata.com
127.0.0.1 cache.emop.emopdata.com
127.0.0.1 registry.emop.emopdata.com
127.0.0.1 datahub-db.emop.emopdata.com
127.0.0.1 keycloak-proxy.emop.emopdata.com

# 数据库集群域名映射
127.0.0.1 emop-db-master.emop.emopdata.com
127.0.0.1 emop-db-slave1.emop.emopdata.com
127.0.0.1 emop-db-slave2.emop.emopdata.com
```
...
> **说明**：认证服务启动后可通过 http://keycloak.emop.emopdata.com:9180/auth 访问管理界面。
...
-   启动脚本会将`emop3/emop-business/emop-server-plugin/target/classes`及`emop3/emop-business/emop-server-plugin-api/target/classes`两个目录下的class文件加入classpath，在IDE中修改源代码后，使用脚本重启服务即可。注意暂时不支持`springboot dev tool`的热加载，遇到class版本的问题，后续会考虑加入`hotswap`
-  Remote Debug 配置说明：
...
:::
#### IDE中启动`plm`服务
-   使用IDE加载 `emop3/emop-business` maven 项目
-   IntelliJ IDEA:
    ...
    -   主类是`emop-business/emop-plm/emop-plm-starter`项目下`io.emop.plm.EmopPLMStarter`

#### IDE中运行集成测试
`emop-server-plugin-integration-test` 用于进行插件的集成测试，集成测试依赖EMOP Server的启动。

:::warning 🔔提醒
所有plugin所有plugin service都应该有对应的集成测试用于保证领域模型及API的质量及稳定性，因为会放到EMOPService中运行
:::

-   位置：`emop3/emop-business/emop-server-plugins/plugin-integration-test`
-   主要功能：
    -   提供插件开发的测试环境
    -   包含常用的测试用例和工具类
    -   支持模拟 PLM 服务的各种场景
...
正常是需要登录的，可以继续往后，把中间件等内容安装完毕后通过 `http://emop.emopdata.com` 页面进行登录
...
# 检查服务可达性
curl -v http://server.emop.emopdata.com:870/webconsole/public/health
```
...
- 检查网关路由：`curl -v -H "Host: emop.emopdata.com" localhost/webconsole/public/health`
- 检查hosts文件配置是否正确
...
- 测试服务间连通性：`docker exec -it emop-server ping keycloak.emop.emopdata.com`
- 测试数据库连通性：`docker exec -it emop-server ping emop-db-master.emop.emopdata.com`