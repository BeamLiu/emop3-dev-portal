...
EMOP3 的 3D 预览页面支持两种认证方式：

#### 1. 直接访问（需要已登录）

如果用户已经在 EMOP3 中登录，可以直接访问 3D 预览页面：

```
https://dev.emop.emopdata.com/web/public-3d.html?model=/minioproxy/api/file-proxy/123/converted
```
...
const accessToken = '...'; // 从 第三方 认证服务获取
```
...
```javascript
// 第三方 中的完整跳转代码
function jumpTo3DViewer(fileId, accessToken, refreshToken) {
  // 构造模型地址
  const modelUrl = `/minioproxy/api/file-proxy/${fileId}/converted`;
  
  // 构造最终目标页面
  const finalTarget = `/web/public-3d.html?model=${encodeURIComponent(modelUrl)}`;
  
  // 构造认证重定向 URL
  const baseUrl = 'https://dev.emop.emopdata.com';
  const redirectUrl = `${baseUrl}/public-auth-redirect.html?access_token=${accessToken}&redirect=${encodeURIComponent(finalTarget)}`;
...
// 使用示例
jumpTo3DViewer('123', 'your_access_token');
```
...
获取 access_token
    ↓
跳转到 /public-auth-redirect.html?access_token=xxx&redirect=/web/public-3d.html?model=xxx
    ↓
存储 token 到 cookie
    ↓
自动跳转到 /web/public-3d.html?model=xxx
    ↓
加载 3D 模型（使用 cookie 中的 token）
```
...
|------|------|------|
| `access_token` | 是 | 访问令牌 |
| `refresh_token` | 否 | 刷新令牌，如果不提供则使用 access_token |
| `redirect` | 否 | 登录成功后要跳转的完整 URL，默认为 `/web/home` |

### 权限控制

- **页面访问**：需要有效的 access_token（通过 cookie 或认证重定向获取）
- **文件加载**：在后端 API 层面控制权限
  - 有权限：正常加载文件
  - 无权限：返回 401/403 错误，显示错误提示

### iframe 嵌入时的认证

如果需要在 iframe 中嵌入 3D 预览页面，有两种方式：

#### 方式1：父页面已登录

如果父页面和 iframe 在同一域名下，且用户已登录，cookie 会自动共享：

```html
<iframe 
  src="https://dev.emop.emopdata.com/web/public-3d.html?model=/minioproxile-proxy/123/converted"
  width="100%" 
  height="600px">
</iframe>
```
...
如果是跨域嵌入，需要先通过认证重定向获取 token：

```javascript
// 在父页面中
function embedAuthenticatedViewer(fileId, accessToken) {
  const modelUrl = `/minioproxy/api/file-proxy/${fileId}/converted`;
  const finalTarget = `/web/public-3d.html?model=${encodeURIComponent(modelUrl)}`;
  
  const iframeSrc = `https://dev.emop.emopdata.com/public-auth-redirect.html?access_token=${accessToken}&redirect=${encodeURIComponent(finalTarget)}`;
